...
def get_news_feeds():
    ...
    feed_urls = [
        ...
        "https://news.google.com/rss/search?q=%E5%86%99%E7%9C%9F%E5%B1%95+%E6%9D%B1%E4%BA%AC+OR+%E5%86%99%E7%9C%9F%E5%B1%95+%E5%9F%BC%E7%8E%89+OR+%E3%82%AD%E3%83%A4%E3%83%8E%E3%83%B3%E3%82%AE%E3%83%A4%E3%83%8E%E3%83%AA%E3%83%BC&hl=ja&gl=JP&ceid=JP:ja",
...
def generate_morning_briefing(news_text, cnbc_text, anzn_text):
    """出力上限を8192文字に拡張し、途切れを完全防止"""
    client = genai.Client(api_key=GEMINI_API_KEY)
    
    prompt = f"""
あなたはトップクラスの金融・時事・防災速報アナリストです。
提供されたデータをもとに、Discord上で極めて視認性が高く、実用的な最新速報サマリーを作成してください。
【最重要】途中で文章が途切れないよう、各セクションは要点を濃縮して簡潔にまとめ、最後の【10. 本日の主要・時事ニュース厳選（7〜8本）】まで必ずすべて出力し切ってください。

【出力時の絶対ルール】
1. **URL表記**: Discordの埋め込みプレビュー展開を防ぐため、すべてのURLは `<https://...>` のように `< >` で囲むこと。
2. **Googleマップ**: `<https://www.google.com/maps/search/?api=1&query=場所名>` の形式で出力すること。
3. **日時の完全明記**: ニュース、気象、スポーツ、カルチャー等には「何月何日 何時何分 発表/開催」を明記すること。
4. **事実のみ記載**: 浦和レッズや各試合結果は確定スコアと次回日程のみを記載。写真展は日本国内（首都圏）限定。
...
【2. 🚨 火災・防災・事故速報】
・埼玉および全国の直近火災・消防出動（発生・発表日時を必ず明記）
...
・CNBC文字起こしからの米国マクロ、FRB動向、個別銘柄議論、著名コメンテーター発言要約（放送日時明記）
...
・必ず1番から7〜8番まで漏れなく出力すること（配信日時、報道元名、見出し、1〜2行要約、元記事 `<URL>`）
==================================================
