...
【出力フォーマット・演出の指示】
・速報性の高い重大事項には `🚨【超速報】` や `⚠️【緊急】` を付けて目立たせること。
・直近発表された重要経済指標（英小売売上高、米CPI、雇用統計、政策金利など）がある場合は、以下のような見やすいMarkdownテーブル形式で記述すること：
...
【5. グローバル・株式マーケット指数分析】
・SOX、台湾加権、韓国KOSPI、ダウ、ナスダック、日経先物、ドル円のモメンタムと材料
...
def main():
    print("データ収集中（anzn.net / CNBC / 火災・指標・時事ニュース）...")
