  ...
  3. Apple/シリコンバレー、珈琲
     ...
     直近数日以内に更新されていることを確認済み)。
       - Apple/シリコンバレー: ITmedia速報(rss.itmedia.co.jp) + GIGAZINE
         ...
         直接関連する記事だけを抽出する。
       - 珈琲: AFRO BLOG(afroaster.com、焙煎士歴12年の個人ブログ) +
         PostCoffeeマガジン(postcoffee.co/magazine)。珈琲専門メディア/
         個人ブログ自身のRSSのためYahoo!転載は原理的に混入しない。
         (当初coffee-station.jpを候補にしていたが、GitHub ActionsのIPから
         一貫して403を返すため不採用にした。)
...
TOPIC_SOURCES = {
    ...
    # 絞り込みは廃止した。PostCoffeeマガジン/AFRO BLOGは店舗開店の地域ニュース
    # ではなく全国向けの一般的な珈琲カルチャー記事(淹れ方・産地解説・焙煎士の
    # 視点等)が中心で、タイトルに地域名が出てくること自体がほぼ無いため、
    # 関東限定フィルタを維持すると実質ほぼ0件配信になってしまうことを実データ
    # (本番GitHub Actions実行)で確認したため。
    # ※coffee-station.jpは当初候補にしていたが、GitHub ActionsのIPから
    # 一貫して403(Cloudflare等によるクラウドIPブロックとみられ、ヘッダー
    # 変更でも回避不可)を返すことを本番実行で確認したため不採用とし、
    # 現役の個人焙煎士ブログ AFRO BLOG(afroaster.com、焙煎士歴12年)に
    # 差し替えた。
    "☕ 珈琲": [
        "https://afroaster.com/feed/",
