...
def main():
    ...
    if had_error:
        ...
        # 新着記事はすでにstateへ書き込み済みだが、ここで保存をスキップする
        # ことで次回実行時に再度「新着」として拾い直せるようにする(2026-08-28、
        # 送信失敗にもかかわらず既読マークだけ確定してしまい記事が消失した
        # 実例を踏まえた対応。複数チャンクの一部だけ送信済みの場合は再送で
        # 重複が起きうるが、記事が完全に失われるよりは安全と判断した)。
        print("[WARN] Discord送信に失敗したため、既送信記録の保存をスキップします"
              "(次回実行時に再送を試みます)。")
        sys.exit(1)
