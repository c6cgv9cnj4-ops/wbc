...
#
# 2026-09-13追加: 文春オンライン等の週刊誌・実話系メディアが「バドミントン」
# キーワードにヒットして「ライバー転身」「投げ銭トラブル」等の芸能・ゴシップ
# 記事を「結果ダイジェスト」として連投する事象が発生したため、細川さんの
# 指定によりドメインブラックリストに週刊誌・ゴシップ系メディアを追加した。
# これらのドメインはタイトルに「バドミントン」を含んでいても is_gnews_noise
# の先頭チェック(ドメイン一致)で無条件に破棄する(キーワード一致は見ない)。
GNEWS_DOMAIN_BLACKLIST = [
    "prtimes.jp", "efight.jp",
    "bunshun.jp",        # 文春オンライン
    "dailyshincho.jp",   # デイリー新潮
    "jisin.jp",          # 女性自身
    "nikkan-gendai.com",  # 日刊ゲンダイDIGITAL
]
# ゴシップ記事がYahoo!ニュース等の転載経由で配信元ドメイン判定をすり抜ける
# ケース向けの内容キーワード除外(2026-09-13、細川さんの指定により追加)。
GNEWS_KEYWORD_BLACKLIST = [
    "パートナーシップ", "独占販売", "クラウドファンディング", "協賛",
    "グラビア", "水着", "美ボディ", "9頭身", "温泉", "【究極の2択】",
    "ライバー転身", "投げ銭", "整形", "告白", "熱愛", "不倫", "スキャンダル", "暴露",
]
