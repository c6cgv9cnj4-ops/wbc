...
logs/daily/YYYY-MM-DD.md ・ logs/health/YYYY-MM-DD.md (当日分)を読み込み、
Anthropic API(Claude)で「日刊サマリー」と「ミニMermaidマップ」を生成し、
reports/daily/YYYY-MM-DD.md に保存したうえでDiscordへ送信する。
...
# 依頼では claude-3-5-sonnet が指定されていたが、Geminiモデルの提供終了で
# 一度ハマった経験(price_bot/scripts/scraper.py参照)を踏まえ、廃止された
# 特定バージョンに固定せず、環境変数で上書きできるようにしたうえで
# 現行の推奨モデルをデフォルトにしている。3.5 Sonnet系を使いたい場合は
# ANTHROPIC_MODEL="claude-3-5-sonnet-latest" 等をSecretsで指定すればよい。
